...
SELECT o.id, p.id, p.name, p.unit, v.qty, v.price
FROM orders o
JOIN (VALUES
  ('DH00001', 'GIN-A4-80',  50, 85000),
  ('DH00002', 'GAN-BONG',   20, 145000),
  ('DH00003', 'VAI-COTTON', 100, 15000),
  ('DH00004', 'GIN-A3-80',  30, 160000),
  ('DH00005', 'GIN-A4-80',  80, 85000),
  ('DH00006', 'LOI-3IN',    50, 8000),
  ('DH00007', 'GAN-MO',     25, 138000),
  ('DH00008', 'GBI-300',   120, 35000),
  ('DH00009', 'GIN-A4-80',  20, 85000),
  ('DH00009', 'GAN-BONG',   10, 145000),
  ('DH00010', 'VAI-POLY',  150, 12000)
) AS v(ocode, pcode, qty, price)
  ON o.code = v.ocode
...
SELECT o.id, v.amount, v.method, v.pdate, 1, NULL
FROM orders o
JOIN (VALUES
  ('DH00001', 4250000, 'transfer', '2026-01-13'),
  ('DH00002', 2900000, 'transfer', '2026-01-19'),
  ('DH00003', 1500000, 'cash',     '2026-01-25'),
  ('DH00004', 4800000, 'transfer', '2026-02-06'),
  ('DH00005', 3000000, 'transfer', '2026-02-13'),
  ('DH00006',  400000, 'cash',     '2026-02-18'),
  ('DH00010', 1800000, 'cash',     '2026-03-20')
) AS v(ocode, amount, method, pdate) ON o.code = v.ocode;
